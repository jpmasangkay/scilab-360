...
const TeacherDashboard: React.FC = () => {
  ...
  return (
    <div style={{
      ...
      background: "#0d0120", border: "1px solid #2d1b5e", zIndex: 1000,
      display: "flex", flexDirection: "column", padding: 16, gap: 12,
      boxShadow: "-4px 0 40px #00000080, -2px 0 20px #a855f720",
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontFamily: "'Orbitron', monospace", fontWeight: 700, fontSize: 13, color: "#e2e8f0", letterSpacing: 2 }}>TEACHER DASHBOARD</div>
        <button onClick={() => dispatch({ type: "TOGGLE_TEACHER" })} style={{ background: "none", border: "none", color: "#4c1d95", cursor: "pointer", fontSize: 18 }}>✕</button>
      </div>

      <div style={{ background: "#130929", borderRadius: 8, padding: 12, border: "1px solid #2d1b5e" }}>
        <div style={{ fontSize: 10, color: "#4c1d95", fontFamily: "'Share Tech Mono', monospace", letterSpacing: 2 }}>CLASS SUMMARY</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}>
          {[
            ["Students", "1"], ["Avg Score", state.score.toString()],
            ["Avg Level", state.level.toString()], ["Completed", `${state.completedChallenges.length}/${QUIZ_LEVELS.length}`],
          ].map(([l, v]) => (
            <div key={l} style={{ background: "#07011a", borderRadius: 6, padding: "6px 8px", border: "1px solid #1e0b3e" }}>
              <div style={{ fontSize: 9, color: "#4c1d95", fontFamily: "'Share Tech Mono', monospace" }}>{l}</div>
              <div style={{ fontSize: 18, color: "#a855f7", fontFamily: "'Orbitron', monospace", fontWeight: 700, textShadow: "0 0 10px #a855f760" }}>{v}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: "#130929", borderRadius: 8, padding: 12, border: "1px solid #2d1b5e" }}>
        <div style={{ fontSize: 10, color: "#4c1d95", fontFamily: "'Share Tech Mono', monospace", marginBottom: 8, letterSpacing: 2 }}>STUDENT #1 PROGRESS</div>
        {QUIZ_LEVELS.map(q => (
          <div key={q.level} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            <span style={{ fontSize: 10, color: "#6b21a8", fontFamily: "'Share Tech Mono', monospace" }}>L{q.level}: {q.targetFormula}</span>
            <span style={{ fontSize: 10, color: state.completedChallenges.includes(q.level) ? "#a855f7" : "#2d1b5e" }}>
      ...
      <div style={{ background: "#130929", borderRadius: 8, padding: 12, border: "1px solid #2d1b5e" }}>
        <div style={{ fontSize: 10, color: "#4c1d95", fontFamily: "'Share Tech Mono', monospace", marginBottom: 8, letterSpacing: 2 }}>SESSION STATS</div>
        <div style={{ fontSize: 11, color: "#6b21a8", fontFamily: "'Share Tech Mono', monospace", lineHeight: 2 }}>
          Atoms Placed: {state.atomCount}<br />
          Attempts: {state.attempts}<br />
          Mode: {state.mode}<br />
          Formula: {state.formula || "—"}<br />
          Bonds: {state.bonds.length}
        </div>
      </div>

      <button onClick={exportReport} style={{
        padding: "10px 0", background: "linear-gradient(135deg, #3b0764, #4c1d95)", border: "1px solid #a855f7", color: "#d8b4fe",
        borderRadius: 8, cursor: "pointer", fontFamily: "'Orbitron', monospace", fontSize: 11, fontWeight: 700,
        marginTop: "auto", letterSpacing: 1,
