  ...
  if (!challenge) {
    return (
      <div style={{ padding: 20, borderRadius: 14, textAlign: 'center', background: theme.accentBg, border: `1px solid ${theme.accentBorder}` }}>
        <p style={{ fontFamily: '"Nunito", sans-serif', fontSize: 16, fontWeight: 800, color: theme.accent }}>All Challenges Complete!</p>
        ...
        <button onClick={() => dispatch({ type: 'SET_LEVEL', payload: 1 })}
          style={{ marginTop: 12, padding: '8px 20px', borderRadius: 10, cursor: 'pointer', fontFamily: '"Nunito", sans-serif', fontSize: 13, fontWeight: 700, background: theme.accent, border: 'none', color: '#ffffff' }}>
          Play Again
        </button>
      </div>
    );
    ...
    <div style={{ borderRadius: 14, overflow: 'hidden', background: theme.surface, border: `1px solid ${theme.border}`, boxShadow: theme.shadow }}>
      ...
      <div onClick={() => setCollapsed(v => !v)}
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, fontFamily: '"Nunito", sans-serif', fontWeight: 800, padding: '3px 10px', borderRadius: 8, background: theme.accent, color: '#ffffff' }}>
            Lvl {challenge.level}
          </span>
          <span style={{ fontSize: 11, fontFamily: '"Nunito", sans-serif', fontWeight: 700, padding: '3px 9px', borderRadius: 8, background: diffColors[challenge.difficulty]?.bg ?? theme.statBg, color: diffColors[challenge.difficulty]?.text ?? theme.textSecondary, border: `1px solid ${diffColors[challenge.difficulty]?.border ?? theme.border}` }}>
            {diff.emoji} {diff.label}
          </span>
          <span style={{ fontSize: 13, fontFamily: '"Nunito", sans-serif', fontWeight: 700, color: theme.text }}>
      ...
      {!collapsed && (
        <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {/* Difficulty selector */}
          <div>
            <p style={{ fontFamily: '"Nunito", sans-serif', fontSize: 11, color: theme.textTertiary, letterSpacing: '0.05em', marginBottom: 7, fontWeight: 700, textTransform: 'uppercase' }}>Difficulty</p>
              ...
              {(['all', 'easy', 'medium', 'hard'] as const).map(d => {
                ...
                return (
                  <button key={d} onClick={() => setActiveDifficulty(d)}
                    style={{ flex: 1, padding: '6px 0', borderRadius: 8, cursor: 'pointer', fontFamily: '"Nunito", sans-serif', fontSize: 12, fontWeight: 700, transition: 'all 0.15s', background: isActive ? dc.bg : theme.surfaceAlt, color: isActive ? dc.text : theme.textTertiary, border: `1px solid ${isActive ? dc.border : theme.border}` }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {Object.entries(atomCounts).map(([sym, { have, need, ok }]) => (
              <div key={sym} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 10, fontFamily: '"Nunito", sans-serif', fontSize: 13, fontWeight: 700, background: ok ? (theme.isDark ? '#0a2a1a' : '#f0fdf4') : theme.surfaceAlt, border: `1px solid ${ok ? (theme.isDark ? '#166534' : '#86efac') : theme.border}`, color: ok ? '#16a34a' : theme.textSecondary }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 11, fontFamily: '"Nunito", sans-serif', fontWeight: 700, letterSpacing: '0.05em', color: theme.textTertiary, textTransform: 'uppercase' }}>Atoms Placed</span>
          ...
          <div style={{ padding: '10px 14px', background: theme.isDark ? '#2a2410' : '#fffbeb', border: `1px solid ${theme.isDark ? '#854d0e' : '#fde68a'}`, borderRadius: 10 }}>
            <p style={{ fontFamily: '"Inter", sans-serif', fontSize: 13, color: theme.isDark ? '#fbbf24' : '#92400e', lineHeight: 1.6 }}>Hint: {challenge.hint}</p>
          </div>

          {/* Submit result */}
          {submitResult && (
            <div style={{ padding: '10px 14px', borderRadius: 10, background: submitResult.ok ? (theme.isDark ? '#0a2a1a' : '#f0fdf4') : (theme.isDark ? '#2a1a1a' : '#fef2f2'), border: `1px solid ${submitResult.ok ? (theme.isDark ? '#166534' : '#86efac') : (theme.isDark ? '#7f1d1d' : '#fecaca')}`, color: submitResult.ok ? '#16a34a' : '#dc2626' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {nextChallenge && (
                <button
                  ...
                  style={{ padding: 12, borderRadius: 12, color: '#ffffff', fontFamily: '"Nunito", sans-serif', fontWeight: 800, fontSize: 14, cursor: 'pointer', border: 'none', background: theme.accent, transition: 'all 0.2s' }}
          ...
          {/* Level selector */}
          <div>
            <p style={{ fontFamily: '"Nunito", sans-serif', fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', color: theme.textTertiary, marginBottom: 8, textTransform: 'uppercase' }}>
              ...
              {filteredLevels.map(q => {
                ...
                return (
                  <button key={q.level}
                    ...
                    style={{ minWidth: 40, padding: '5px 10px', borderRadius: 8, fontSize: 12, cursor: 'pointer', fontFamily: '"Nunito", sans-serif', fontWeight: 700, transition: 'all 0.15s', background: isActive ? theme.accent : isDone ? dc.bg : theme.surfaceAlt, color: isActive ? '#ffffff' : isDone ? dc.text : theme.textTertiary, border: isActive ? `1px solid ${theme.accentDark}` : `1px solid ${isDone ? dc.border : theme.border}`, display: 'flex', alignItems: 'center', gap: 4 }}>
                    {q.level}
